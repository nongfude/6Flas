...
bounded by the MAC and the $32$-bit tag. A reproducible processing model, an
event-driven model of a 1584-satellite constellation, and a Python software
prototype analytically project that 6FLAS cuts per-hop processing about
$48\times$ versus IPsec ESP and $5.9\times$ versus an SRv6 HMAC baseline; the
prototype confirms the structural ordering on commodity hardware. Together these
results indicate 6FLAS reduces per-packet overhead by $79\%$, eliminates
per-flow on-board state (zero bytes per flow), and keeps end-to-end latency
within the propagation-dominated regime. These
