...
surface grows with every added hop, while the per-hop security budget is severely constrained.
...
stateless DoS, and quantify cost with an analytical processing model, an
event-driven latency model of a 1584-satellite constellation, and a Python
software prototype
...
Analytically projected on a 1\,GHz space-grade core, 6FLAS verifies a packet in
about $306$\,ns per hop, roughly $48\times$ faster than IPsec ESP and $5.9\times$
faster than an SRv6 HMAC baseline; it adds only $8$ bytes per packet ($79\%$
less than IPsec ESP) and carries zero per-flow on-board state ($100\%$ reduction
versus IPsec~ESP; total fixed state is $192$\,B, direction-scoped). Under a
$10^6$-flow load its single-core forwarding throughput is analytically projected
at about $48\times$ that of IPsec ESP, and its end-to-end latency stays within
the propagation-dominated regime of the constellation.
