...
\begin{tikzpicture}[
    sat/.style={circle,draw,fill=blue!14,minimum size=22pt,inner sep=0pt,font=\tiny},
    ep/.style={rectangle,draw,rounded corners,fill=gray!12,
               minimum width=1.1cm,minimum height=0.52cm,font=\tiny,align=center},
    ctrl/.style={rectangle,draw,rounded corners,fill=orange!10,
                 minimum width=4.4cm,minimum height=0.55cm,font=\tiny,align=center},
    pkt/.style={->,thick,blue!55},
    key/.style={->,dashed,orange!60,thin},
    ann/.style={font=\fontsize{5}{6}\selectfont,align=center},
    >=Latex
]
%% Control plane
\node[ctrl] (ctrl) at (3.8,2.8)
  {Ground Controller:\\[-2pt]provisions $\skey_\domid$;\enspace programs SRH path via Security SID};

%% Data-plane nodes
\node[ep]  (src) at (0.0, 0) {Source $A$};
\node[sat] (v1)  at (1.9, 0) {$v_1$};
\node[sat] (v2)  at (3.8, 0) {$v_2$};
\node[sat] (v3)  at (5.7, 0) {$v_3$};
\node[ep]  (gw)  at (7.6, 0) {GW $G$};
...
%% Key provisioning (controller → every node)
\draw[key] (ctrl) -- (src.north east);
\draw[key] (ctrl) -- (v1.north);
\draw[key] (ctrl) -- (v2.north);
\draw[key] (ctrl) -- (v3.north);
\draw[key] (ctrl) -- (gw.north west);

%% Source annotation
\node[ann,below=4pt of src]
  {$\fl=\prf_{\skey_\domid}(\textit{5t}\,\|\,\domid)$\\
   $\mtag=\mac_{\skey_\domid}(\fl\|\textrm{SID}\|\domid\|\seq)|_{32}$};

%% Satellite annotations
\node[ann,below=4pt of v1]{\texttt{End.SEC}\\parse$\to$replay\\$\to$MAC$\to$dir};
\node[ann,below=4pt of v2]{\texttt{End.SEC}\\parse$\to$replay\\$\to$MAC$\to$dir};
\node[ann,below=4pt of v3]{\texttt{End.SEC}\\parse$\to$replay\\$\to$MAC$\to$dir};

%% Gateway annotation
\node[ann,below=4pt of gw]{deliver\\(no SA)};

%% Packet-field label on the wire
\node[ann,above=3pt of v2]{$[\,\fl\;\big|\;\textrm{SID}\;\big|\;\domid\;\big|\;\seq\;\big|\;\mtag\,]$};
\end{tikzpicture}%
}
\caption{6FLAS end-to-end architecture. The ground controller provisions the
domain key $\skey_\domid$ to the source and all on-board routers and programs the
SRH path. The source computes the Flow Label and the 32-bit MAC tag; each
on-board router executes the four-step \texttt{End.SEC} procedure
(\cref{alg:verify}) using only fields in the packet and the local domain key —
no per-flow state exists to migrate on an ISL handover.}
\label{fig:arch}
\end{figure}
