...
tag. Each on-board router verifies the tag over the shaded routing span and
...
\texttt{End.SEC} is designed as a strict superset of the \texttt{End} behavior:
a node that implements \texttt{End} but not \texttt{End.SEC} performs standard SRv6
forwarding without per-hop security verification, preserving connectivity. Backward
compatibility therefore holds within a deployment where all active-path nodes
implement at least the base \texttt{End} behavior.
